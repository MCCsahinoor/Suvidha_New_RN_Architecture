...
import { View, ScrollView, Text, ImageBackground, Pressable, RefreshControl, Linking, ActivityIndicator, } from 'react-native';
...
const ProductTutorials = ({ navigation }: any) => {
    ...
    useEffect(() => {
        if (status == 'succeeded' && (product_tutorial ?? '')) {
            setTutorialsList(product_tutorial);
        }
    }, [product_tutorial]);
                                ...
                                <Card>
                                    <Pressable style={{ ...styles.playerList }} onPress={() => { download(item.doc_link, index); }}>
                                        <View style={{ width: 100 }}>
                                            <ImageBackground source={{ uri: AppImages.Paper }} resizeMode='contain' style={{ ...styles.backgroundContainerCoverThumaNail }} imageStyle={{ borderRadius: 8 }} >
